$(document).ready(function() {
    ...
    function buttonDisable(elem) {
        ...
        elem.parents('.form').find('input, textarea').not('input[type="hidden"], input#photo-upload-input3').each(function() {
            if ($(this).val().length == 0 || (elem.parents('.form').find('.checkbox input').length > 0 && (!(elem.parents('.form').find('.checkbox input').is(':checked'))))) {
                empty = true;
            }
        });
        ...
        if (empty) {
            elem.parents('.form').find('[type="submit"]').addClass('btn--disabled').attr('disabled', 'disabled');
        } else {
            elem.parents('.form').find('[type="submit"]').removeClass('btn--disabled').attr('disabled', false);
        }
    }
